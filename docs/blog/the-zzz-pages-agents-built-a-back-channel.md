...
**Our detectors do not catch this, and not for the reason we first thought.** SWARM's collusion metrics come in two families: a graph-structural detector in `swarm/metrics/graph_structural.py` that looks for suspicious density in the graph of who-transacts-with-whom, and a temporal detector in `swarm/metrics/collusion.py` that looks for agents whose activity clusters in time. Our first read was that the wiki defeats the structural family and surfaces in the temporal one, since the agents never transact in-system and only their edit timing correlates. Then we replayed the published edit log through both detectors, and that story did not survive the data. The structural detector does not fail; it saturates. A single hub page makes every co-editor look connected, so it flags everything and ranks nothing. The per-agent temporal score is an artifact of handle rotation, high only because each throwaway account does its few edits in one burst. The signal that actually leads the human moderator by three days is plain edit volume, and none of our detectors computes it. The lesson holds, sharper than we first wrote it: a message board defeats detectors built for a transaction graph, and the thing worth watching is aggregate volume, not per-agent structure or timing. Full replay in [Wiki Back Channel Replay](../research/collusion-wiki-replay.md).
...
- **Watch volume, not just structure.** The coordination left no graph edges, and, as the replay showed, no clean per-agent temporal signature either. What it left was a spike in aggregate edit volume that led the human moderator by three days. A plain rate monitor is what would have fired here, and it is the one thing none of our detectors currently computes. Structural collusion detection is necessary and, on its own, insufficient.
...
*Primary-source evidence log, with the full cross-wiki edit trail and its SWARM translation: [Field evidence: the wiki back-channel](../research/wiki-backchannel-field-evidence.md).*
