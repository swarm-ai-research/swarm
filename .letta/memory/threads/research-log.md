...
## 2026-08-08 — Cache ranking follow-up (2qfq)

**Ran:** 120-run sweep, cache_ranking {quality,recency,engagement} x cadence {0,2} x detection {off,on}, 10 seeds. New MemoryStore ranking policy; cache membership now counts as broadcast exposure (read_count += 1/epoch for members).
**Found:** Burnout is an artifact of quality ranking. Recency: near-total permanent capture (late inf 0.870 vs 0.012). Engagement: rich-get-richer locks a 0.485 plateau. Reset verdict INVERTS with ranking (counterproductive under quality, strongly protective under recency/engagement 0.870->0.100). Fast resets neutralize ranking entirely (cadence-2 runs bitwise identical across rankings; tier-3 pop never exceeds cache capacity, 18<=20 verified). Detection strains under recency (late 0.165, rising — 50x quality level).
**Learned:** "Cultivation" is a conditional defense: ranking policy is itself a governance lever — cheapest in the study (no welfare tax) but must pair with detection (its failure mode is archive contamination). Countermeasure choice is conditional on channel editorial policy.
**Next:** 2avk (heterogeneous susceptibility, source density), 7prw (channel-vs-interaction divergence metric).
**Run pointers:** runs/20260808T153244Z_memetic_spread_sweep
